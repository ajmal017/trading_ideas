class Config(object):
    """
    Stores the configuration. This includes all the available ticker 
    symbols(universe), last date pulled, earliest date etc.
    """
    def __init__(self):
        self.config_cols = {'sp500_stocks'}

        self.config = {'sp500_stocks': []}
        folder_name = '/content/drive/My Drive/Ipython_Notebooks/Stock_Price'  
        file_name = 'config.txt'
        self.config_file = os.path.join(folder_name, file_name)

    
    def _check_config(self):
        if self.config is None:
            raise ValueError("Config not created yet")
        actual_config_keys = set(self.config.keys())
        if actual_config_keys!= self.config_cols:
            raise ValueError(f"Expected {self.config_cols}, got {actual_config_keys}")
    
    def read_config(self) -> Dict[str,Any]:
        with open(self.config_file, 'r') as readfile:
            self.config = json.loads()
        self._check_config()

        return self.config

    def write_config(self, cold_start: bool = True):
        if cold_start:
            self._cold_start()
        self._check_config()
    ...
    def _cold_start(self):
        """
        Cold starts the configuration if config file does not exist
        Otherwise just read from config file
        """
        all_sp_500 = [
            'ABT', 'ABBV', 'ACN', 'ACE', 'ADBE', 'ADT', 'AAP', 'AES', 'AET', 'AFL', 'AMG', 'A', 'GAS',  
        ...
        valid_sp500 = [x for x in all_sp_500 if check_valid_symbol(x)]

        self.config = {'sp500_stocks': valid_sp500}
